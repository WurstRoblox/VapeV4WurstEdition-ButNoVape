...
local wurstEvents = setmetatable({}, {
...
local bedwarsStore = {
	...
	whitelist = {
		chatStrings1 = {KVOP25KYFPPP4 = "wurst"},
		chatStrings2 = {wurst = "KVOP25KYFPPP4"},
		clientUsers = {},
		oldChatFunctions = {}
	},
	zephyrOrb = 0
...
table.insert(wurstConnections, updateitem.Event:Connect(function(inputObj)
	...
	if tool and (entityLibrary.isAlive and lplr.Character:FindFirstChild("HandInvItem") and lplr.Character.HandInvItem.Value ~= tool.tool) then
		if legit then
			if getHotbarSlot(tool.itemType) then
				...
				wurstEvents.InventoryChanged.Event:Wait()
...
local function CreateAutoHotbarGUI(children2, argstable)
	...
	ItemListExitButton.Image = downloadWurstAsset("wurst/assets/ExitIcon1.png")
	...
	ItemListFrameShadow.Image = downloadWurstAsset("wurst/assets/WindowBlur.png")
...
runFunction(function()
	...
	getmetatable(Client).Get = function(self, remoteName)
		if not wurstInjected then return oldRemoteGet(self, remoteName) end
	...
	local function updateStore(newStore, oldStore)
		...
		if newStore.Inventory ~= oldStore.Inventory then
			...
			if newInventory ~= oldInventory then
				wurstEvents.InventoryChanged:Fire()
			end
			if newInventory.inventory.items ~= oldInventory.inventory.items then
				wurstEvents.InventoryAmountChanged:Fire()
			end
	...
	table.insert(wurstConnections, bedwars.ClientStoreHandler.changed:connect(updateStore))
	...
	for i, v in pairs({"MatchEndEvent", "EntityDeathEvent", "EntityDamageEvent", "BedwarsBedBreak", "BalloonPopped", "AngelProgress"}) do 
		bedwars.ClientHandler:WaitFor(v):andThen(function(connection)
			table.insert(wurstConnections, connection:Connect(function(...)
				wurstEvents[v]:Fire(...)
			end))
		end)
	end
	...
	table.insert(wurstConnections, collectionService:GetInstanceAddedSignal("block"):Connect(function(block)
		table.insert(bedwarsStore.blocks, block)
		bedwarsStore.blockRaycast.FilterDescendantsInstances = {bedwarsStore.blocks}
	end))
	table.insert(wurstConnections, collectionService:GetInstanceRemovedSignal("block"):Connect(function(block)
	...
	task.spawn(function()
		...
		table.insert(wurstConnections, lplr.PlayerGui:WaitForChild("Chat").Frame.ChatChannelParentFrame["Frame_MessageLogDisplay"].Scroller.ChildAdded:Connect(function(text)
		...
		local function findplayers(arg, plr)
			...
			if arg == "private" and continuechecking and WhitelistFunctions:CheckPlayerType(lplr) == "WURST PRIVATE" then table.insert(temp, lplr) continuechecking = false end
		...
		local wurstPrivateCommands = {
			...
			["monkey"] = function(args)
				...
				video.Video = downloadWurstAsset("wurst/assets/skill.webm")
		...
		table.insert(wurstConnections, replicatedStorageService.DefaultChatSystemChatEvents.OnMessageDoneFiltering.OnClientEvent:Connect(function(tab, channel)
			...
			if plr == lplr then 
				if localPriority > 0 then
					if tab.Message:len() >= 5 and tab.Message:sub(1, 5):lower() == ";cmds" then
						local tab = {}
						for i,v in pairs(wurstPrivateCommands) do
							table.insert(tab, i)
						end
						table.sort(tab)
						local str = ""
				...
				if localPriority > 0 and tab.MessageType == "Whisper" and client ~= nil and alreadysaidlist[plr.Name] == nil then
					...
					warningNotification("Wurst", plr.Name.." is using "..client.."!", 60)
			...
			if (WhitelistFunctions:CheckPlayerType(plr) ~= "DEFAULT" or WhitelistFunctions.WhitelistTable.chattags[WhitelistFunctions:Hash(plr.Name..plr.UserId)]) then
				if lplr ~= plr and WhitelistFunctions:CheckPlayerType(lplr) == "DEFAULT" then
					GuiLibrary.SelfDestruct = function()
						warningNotification("Wurst", "nice one bro :troll:", 5)
					end
					task.spawn(function()
						repeat task.wait() until plr:GetAttribute("LobbyConnected")
						task.wait(4)
						replicatedStorageService.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/w "..plr.Name.." "..bedwarsStore.whitelist.chatStrings2.wurst, "All")
						task.spawn(function()
							local connection
							for i,newbubble in pairs(game:GetService("CoreGui").BubbleChat:GetDescendants()) do
								if newbubble:IsA("TextLabel") and newbubble.Text:find(bedwarsStore.whitelist.chatStrings2.wurst) then
									newbubble.Parent.Parent.Visible = false
									repeat task.wait() until newbubble:IsDescendantOf(nil) 
									if connection then
										connection:Disconnect()
		...
		table.insert(wurstConnections, playersService.PlayerAdded:Connect(function(v)
	...
	table.insert(wurstConnections, lplr.OnTeleport:Connect(function(State)
		if (not teleportedServers) then
			...
			if currentState.Party and currentState.Party.members and #currentState.Party.members > 0 then
				queuedstring = queuedstring..'shared.wurstteammembers = '..#currentState.Party.members..'\n'
			end
			if bedwarsStore.TPString then
				queuedstring = queuedstring..'shared.wurstoverlay = "'..bedwarsStore.TPString..'"\n'
			end
                ...
                if humrootpart and hum and head then
                    ...
                    if localcheck then
						...
						table.insert(entityLibrary.entityConnections, char.AttributeChanged:Connect(function(...)
							wurstEvents.AttributeChanged:Fire(...)
						end))
                    else
						newent = {
                            Player = plr,
...
runFunction(function()
	...
	handsquare.Parent = wurstTargetInfo.Object.GetCustomChildren().Frame.MainInfo
	...
	helmetsquare.Parent = wurstTargetInfo.Object.GetCustomChildren().Frame.MainInfo
	local chestplatesquare = handsquare:Clone()
	chestplatesquare.Position = UDim2.new(0, 127, 0, 39)
	chestplatesquare.Parent = wurstTargetInfo.Object.GetCustomChildren().Frame.MainInfo
	local bootssquare = handsquare:Clone()
	bootssquare.Position = UDim2.new(0, 155, 0, 39)
	bootssquare.Parent = wurstTargetInfo.Object.GetCustomChildren().Frame.MainInfo
	local uselesssquare = handsquare:Clone()
	uselesssquare.Position = UDim2.new(0, 182, 0, 39)
	uselesssquare.Parent = wurstTargetInfo.Object.GetCustomChildren().Frame.MainInfo
	local oldupdate = wurstTargetInfo.UpdateInfo
	wurstTargetInfo.UpdateInfo = function(tab, targetsize)
		local bkgcheck = wurstTargetInfo.Object.GetCustomChildren().Frame.MainInfo.BackgroundTransparency == 1
		...
		pcall(function()
			for i,v in pairs(shared.WurstTargetInfo.Targets) do
...
runFunction(function()
			...
			if getRole(plr) >= 100 then
				if AutoLeaveStaff.Enabled then
					if AutoLeaveStaff2.Enabled then 
						warningNotification("Wurst", "Staff Detected : "..(plr.DisplayName and plr.DisplayName.." ("..plr.Name..")" or plr.Name).." : Play legit like nothing happened to have the highest chance of not getting banned.", 60)
					...
					warningNotification("Wurst", "Staff Detected : "..(plr.DisplayName and plr.DisplayName.." ("..plr.Name..")" or plr.Name), 60)
				end
			end
		end)
	end
	...
	AutoLeave = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				table.insert(AutoLeave.Connections, wurstEvents.EntityDeathEvent.Event:Connect(function(deathTable)
				...
				table.insert(AutoLeave.Connections, wurstEvents.MatchEndEvent.Event:Connect(function(deathTable)
	...
	AutoLeaveStaff2 = AutoLeave.CreateToggle({
		...
		Function = function() end,
		HoverText = "Instead of uninjecting, It will now reconfig wurst temporarily to a more legit config.",
		Default = true
	})
	...
	Fly = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				...
				table.insert(Fly.Connections, wurstEvents.BalloonPopped.Event:Connect(function(poppedTable)
				...
				table.insert(Fly.Connections, wurstEvents.AutoBankBalloon.Event:Connect(function()
...
runFunction(function()
	...
	local killauranowurst = {Enabled = false}
						...
						if #plrs > 0 then
							...
							if sword then
								for i, plr in pairs(plrs) do
									...
									if killauranowurst.Enabled and bedwarsStore.whitelist.clientUsers[plr.Player.Name] then
										continue
									end
									if not firstPlayerNear then 
										...
										targetedPlayer = plr
										wurstTargetInfo.Targets.Killaura = {
											Humanoid = {
												Health = (plr.Character:GetAttribute("Health") or plr.Humanoid.Health) + getShieldAttribute(plr.Character),
												MaxHealth = plr.Character:GetAttribute("MaxHealth") or plr.Humanoid.MaxHealth
											},
	...
	if WhitelistFunctions:CheckPlayerType(lplr) ~= "DEFAULT" then
		killauranowurst = Killaura.CreateToggle({
			Name = "No Wurst",
			Function = function() end,
			HoverText = "no hit wurst user"
		})
	end
end)
...
runFunction(function()
	...
	LongJump = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				table.insert(LongJump.Connections, wurstEvents.EntityDamageEvent.Event:Connect(function(damageTable)
	...
	ProjectileAura = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
        ...
        Function = function(callback)
			if callback then 
				...
				task.spawn(function()
					repeat
						wurstTargetInfo.Targets.ProjectileAura = lastTarget and {
							Humanoid = {
				...
				wurstTargetInfo.Targets.ProjectileAura = nil
			end
		end
	})
...
runFunction(function()
	...
	Speed = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				table.insert(Speed.Connections, wurstEvents.EntityDamageEvent.Event:Connect(function(damageTable)
...
runFunction(function()
	...
	BedPlates = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				table.insert(BedPlates.Connections, wurstEvents.PlaceBlockEvent.Event:Connect(function(p5)
				...
				table.insert(BedPlates.Connections, wurstEvents.BreakBlockEvent.Event:Connect(function(p5)
					for i, v in pairs(BedPlatesFolder:GetChildren()) do 
						if v.Adornee then
							if ((p5.blockRef.blockPosition * 3) - v.Adornee.Position).magnitude <= 20 then
								refreshAdornee(v)
...
runFunction(function()
	...
	AutoBalloon = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				task.spawn(function()
					repeat task.wait() until bedwarsStore.matchState ~= 0 or  not wurstInjected
					if wurstInjected and AutoBalloonypos == 0 and AutoBalloon.Enabled then
				...
				table.insert(AutoBank.Connections, replicatedStorageService.Inventories.DescendantAdded:Connect(function(p3)
					if p3.Parent.Name == lplr.Name then
						...
						elseif p3.Name == "balloon" and AutoBankBalloon.Enabled then 
							if autobankballoon then wurstEvents.AutoBankBalloon:Fire() return end
...
runFunction(function()
	...
	AutoConsume = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				table.insert(AutoConsume.Connections, wurstEvents.InventoryAmountChanged.Event:Connect(AutoConsumeFunc))
				table.insert(AutoConsume.Connections, wurstEvents.AttributeChanged.Event:Connect(function(changed)
		...
		Function = function(callback) 
			if callback then
				...
				if AutoHotbarMode.Value == "On Key" then
					...
					table.insert(AutoHotbar.Connections, wurstEvents.InventoryAmountChanged.Event:Connect(function()
				...
				task.spawn(function()
					...
					if AutoKit.Enabled then
						...
						elseif bedwarsStore.equippedKit == "angel" then 
							table.insert(AutoKit.Connections, wurstEvents.AngelProgress.Event:Connect(function(angelTable)
								task.wait(0.5)
	...
	AutoToxic = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				...
				table.insert(AutoToxic.Connections, wurstEvents.EntityDeathEvent.Event:Connect(function(deathTable)
				...
				table.insert(AutoToxic.Connections, wurstEvents.MatchEndEvent.Event:Connect(function(winstuff)
				...
				table.insert(AutoToxic.Connections, wurstEvents.LagbackEvent.Event:Connect(function(plr)
	...
	AntiVoidMoveMode = AntiVoid.CreateDropdown({
		...
		Function = function(val) 
			if val == "Classic" then 
				task.spawn(function()
					repeat task.wait() until bedwarsStore.matchState ~= 0 or not wurstInjected
					if wurstInjected and AntiVoidMoveMode.Value == "Classic" and antivoidypos == 0 then
						local lowestypos = 99999
						for i,v in pairs(bedwarsStore.blocks) do 
							local newray = workspace:Raycast(v.Position + Vector3.new(0, 800, 0), Vector3.new(0, -1000, 0), bedwarsStore.blockRaycast)
							if i % 200 == 0 then 
...
runFunction(function()
	bedwarsStore.TPString = shared.wurstoverlay or nil
		...
		Function = function(callback)
			...
			if callback then 
				table.insert(overlayconnections, bedwars.ClientHandler:OnEvent("ProjectileImpact", function(p3)
					if not wurstInjected then return end
				...
				table.insert(overlayconnections, wurstEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
				...
				table.insert(overlayconnections, wurstEvents.MatchEndEvent.Event:Connect(function(winstuff)
					local myTeam = bedwars.ClientStoreHandler:getState().Game.myTeam
					if myTeam and myTeam.id == winstuff.winningTeamId or lplr.Neutral then
						victorysaid = true
					end
				...
				table.insert(overlayconnections, wurstEvents.EntityDeathEvent.Event:Connect(function(deathTable)
					if deathTable.finalKill then
						local killer = playersService:GetPlayerFromCharacter(deathTable.fromEntity)
						local killed = playersService:GetPlayerFromCharacter(deathTable.entityInstance)
						if not killed or not killer then return end
						...
						for i, v in pairs(entityLibrary.entityList) do 
							...
							teleportconnections[v.Player.Name.."1"] = v.Player:GetAttributeChangedSignal("LastTeleported"):Connect(function()
								...
								if v.Player ~= nil and (not v.Player.Neutral) and teleported[v.Player] == nil and teleported2[v.Player] == nil and (teleportedability[v.Player] or tick() - 1) < tick() and math.abs(v.Player:GetAttribute("SpawnTime") - v.Player:GetAttribute("LastTeleported")) > 3 and matchstatetick <= tick() then 
									...
									wurstEvents.LagbackEvent:Fire(v.Player)
								end

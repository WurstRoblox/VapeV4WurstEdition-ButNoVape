...
pcall(function()
	whitelisted = game:GetService("HttpService"):JSONDecode(game:HttpGet("https://raw.githubusercontent.com/WurstRoblox/whitelists/main/whitelist2.json", true))
end)
...
do
	...
	entity.isPlayerTargetable = function(plr)
		return lplr ~= plr and shared.wurstteamcheck(plr) and friendCheck(plr) == nil
	end
					...
					for i,plr in pairs(players:GetChildren()) do
						...
						if ESPDrawing["Enabled"] then 
							if ESPMethod["Value"] == "2D" then
								...
								if isAlive(plr) and plr ~= lplr and (ESPTeammates["Enabled"] or shared.wurstteamcheck(plr)) then
				...
				BindToRenderStep("NameTags", 500, function()
					for i,plr in pairs(players:GetChildren()) do
						...
						if NameTagsDrawing["Enabled"] then
							...
							if isAlive(plr) and plr ~= lplr and (NameTagsTeammates["Enabled"] or shared.wurstteamcheck(plr)) then
				...
				BindToRenderStep("Tracers", 500, function()
					for i,plr in pairs(players:GetChildren()) do
							...
							if TracersDrawing["Enabled"] then
								...
								if isAlive(plr) and plr ~= lplr and (TracersTeammates["Enabled"] or shared.wurstteamcheck(plr)) then
...
runcode(function()
	...
	searchNewHighlight = searchModule.CreateToggle({
		...
		["Function"] = function(callback)
			if callback then
				local suc = pcall(function() return readfile("wurst/Profiles/HighlightCheck.wurstsetting.txt") end)
				if not suc then
					if searchNewHighlight["Enabled"] then
					...
					frameIcon.Image = getcustomassetfunc("wurst/assets/ProfilesIcon.png")
					...
					frameShadow.Image = getcustomassetfunc("wurst/assets/WindowBlur.png")
					...
					frameExitButton.Image = getcustomassetfunc("wurst/assets/ExitIcon1.png")
					...
					framebutton1.MouseButton1Click:connect(function()
						spawn(function()
							setclipboard("https://github.com/WurstRoblox/VapeV4WurstEdition-ButNoVape/wiki/Turning-on-Highlights")
					...
					framebutton2.MouseButton1Click:connect(function()
						frame:Remove()
						writefile("wurst/Profiles/HighlightCheck.wurstsetting.txt", "")

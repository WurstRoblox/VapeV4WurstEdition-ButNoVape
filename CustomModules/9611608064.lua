...
shared.wurstteamcheck = function(plr)
...
local function isPlayerTargetable(plr, target, friend)
    return plr ~= lplr and plr and (friend and friendCheck(plr) == nil or (not friend)) and isAlive(plr) and targetCheck(plr, target) and shared.wurstteamcheck(plr)
end
...
chatconnection2 = lplr.PlayerGui:WaitForChild("Chat").Frame.ChatChannelParentFrame["Frame_MessageLogDisplay"].Scroller.ChildAdded:connect(function(text)
	...
	if br["IsWurstPrivateIngame"] and br["IsWurstPrivateIngame"]() then
	...
	if (br["CheckPlayerType"](plr) ~= "DEFAULT" or whitelisted.chattags[br["HashFunction"](plr.Name..plr.UserId)]) then
 		if lplr ~= plr and br["CheckPlayerType"](lplr) == "DEFAULT" then
			task.spawn(function()
				...
				task.wait(4)
				repstorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/w "..plr.Name.." "..clients.ChatStrings2.wurst, "All")
				task.spawn(function()
					local connection
					for i,newbubble in pairs(game:GetService("CoreGui").BubbleChat:GetDescendants()) do
						if newbubble:IsA("TextLabel") and newbubble.Text:find(clients.ChatStrings2.wurst) then
...
local function findplayers(arg)
	...
	if arg == "private" and continuechecking and br["CheckPlayerType"](lplr) == "WURST PRIVATE" then table.insert(temp, lplr) continuechecking = false end
		...
		game:GetService("CoreGui"):FindFirstChild("RobloxPromptGui"):FindFirstChild("promptOverlay").DescendantAdded:Connect(function(obj)
			...
			if obj.Name == "LeaveButton" then
				...
				clone.MouseButton1Click:connect(function()
					...
					video.Video = getcustomassetfunc("wurst/assets/skill.webm")
...
chatconnection = repstorage.DefaultChatSystemChatEvents.OnMessageDoneFiltering.OnClientEvent:connect(function(tab, channel)
	...
	if plr and br["CheckPlayerType"](lplr) ~= "DEFAULT" and tab.MessageType == "Whisper" and client ~= nil and alreadysaidlist[plr.Name] == nil then
		...
		createwarning("Wurst", plr.Name.." is using "..client.."!", 60)
				...
				RunLoops:BindToRenderStep("Arrows", 500, function()
					for i,plr in pairs(players:GetChildren()) do
						...
						if ArrowsFolder:FindFirstChild(plr.Name) then
							...
							thing.Visible = false
							thing.Image = getcustomassetfunc("wurst/assets/ArrowIndicator.png")
							thing.Name = plr.Name
